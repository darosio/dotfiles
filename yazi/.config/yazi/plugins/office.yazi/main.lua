...
--- Local fork of macydnah/office.yazi @ 41ebef8 (upstream HEAD, unmaintained).
--- Ported to the Yazi 26 plugin API, which removed the calls upstream still makes:
---   ya.preview_widgets() -> ya.preview_widget()   (crashed every peek)
---   ya.manager_emit()    -> ya.mgr_emit()         (would crash on seek)
--- Preload failures now render the error instead of leaving a blank pane.
--- Drop this fork and go back to `ya pkg add macydnah/office` if upstream revives.
